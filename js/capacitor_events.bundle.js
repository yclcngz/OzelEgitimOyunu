var CapEvents = (() => {
  ...
  // js/capacitor_events.js
  var capacitor_events_exports = {};
  __export(capacitor_events_exports, {
    initCapacitorEvents: () => initCapacitorEvents
  });
  ...
  // js/capacitor_events.js
  function initCapacitorEvents(bgmAudio, isMutedGetter) {
    ...
    App.addListener("backButton", () => {
      ...
      if (isMainMenu) {
        const now = Date.now();
        if (now - backPressedTime < 2e3) {
          App.exitApp();
        } else {
          backPressedTime = now;
          showToast("\xC7\u0131kmak i\xE7in tekrar bas\u0131n");
        }
      } else {
        window.history.back();
      }
  ...
  return __toCommonJS(capacitor_events_exports);
})();
